...
function useCat() {
  ...
  async function obtenerRazasMascotas() {
    const apiKey = import.meta.env.VITE_API_KEY_CAT;
   ...
   async function obtenerListaImagenesRaza() {
    const apiKey = import.meta.env.VITE_API_KEY_CAT;
    const url = `https://api.thecatapi.com/v1/images/search?breed_ids=${razaSeleccionada}&page=1&limit=20&api_key=${apiKey}`
    try {
      const {data} = await axios(url)
      console.log(data)
